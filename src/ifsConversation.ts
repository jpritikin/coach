...
function stanceChart(
    ...
    svg += `<rect x="${regX.toFixed(1)}" y="0" width="${regW.toFixed(1)}" height="${axisY}" fill="#d4edda" opacity="0.5"/>`;

    // Axis
    svg += `<line x1="0" y1="${axisY}" x2="${W}" y2="${axisY}" stroke="#bbb" stroke-width="1"/>`;
    svg += `<line x1="${(W/2).toFixed(1)}" y1="${axisY-2}" x2="${(W/2).toFixed(1)}" y2="${axisY+2}" stroke="#aaa" stroke-width="1"/>`;
    svg += `<text x="1" y="${H-1}" font-size="8" fill="#aaa">${mirror ? '+1' : '−1'}</text>`;
    svg += `<text x="${(W/2).toFixed(1)}" y="${H-1}" text-anchor="middle" font-size="8" fill="#aaa">0</text>`;
    svg += `<text x="${W-1}" y="${H-1}" text-anchor="end" font-size="8" fill="#aaa">${mirror ? '−1' : '+1'}</text>`;

    // Onion skin: previous positions, oldest → most transparent/smaller
    for (let i = 0; i < history.length; i++) {
        const age = history.length - i; // 1 = most recent
        const t = age / ONION_MAX; // 0=newest, 1=oldest
        const opacity = (Math.sqrt(1 - t) * 0.55).toFixed(2);
        const r = (5 * (1 - t * 0.6)).toFixed(1);
        const x = toX(history[i]);
        svg += `<circle cx="${x.toFixed(1)}" cy="${(axisY - 10).toFixed(1)}" r="${r}" fill="${color}" opacity="${opacity}"/>`;
    }
    ...
    if (shockMag > 0.001) {
        ...
        const tickH = 5;
        svg += `<line x1="${x1.toFixed(1)}" y1="${(bracketY - tickH).toFixed(1)}" x2="${x2.toFixed(1)}" y2="${(bracketY - tickH).toFixed(1)}" stroke="${color}" stroke-width="1.5" opacity="0.4"/>`;
        svg += `<line x1="${x1.toFixed(1)}" y1="${(bracketY - tickH - 3).toFixed(1)}" x2="${x1.toFixed(1)}" y2="${(bracketY + tickH - 3).toFixed(1)}" stroke="${color}" stroke-width="1.5" opacity="0.4"/>`;
        svg += `<line x1="${x2.toFixed(1)}" y1="${(bracketY - tickH - 3).toFixed(1)}" x2="${x2.toFixed(1)}" y2="${(bracketY + tickH - 3).toFixed(1)}" stroke="${color}" stroke-width="1.5" opacity="0.4"/>`;
    }

    // Raw (uncorrected) stance arrow: hollow circle with arrow pointing from effective → raw
    if (rawStance !== undefined && Math.abs(rawStance - stance) > 0.01) {
        const rx = toX(rawStance);
        const cy2 = axisY - 10;
        svg += `<circle cx="${rx.toFixed(1)}" cy="${cy2.toFixed(1)}" r="5" fill="none" stroke="${color}" stroke-width="1.5" opacity="0.55"/>`;
        // Arrow from effective to raw
        const ex = toX(stance);
        const dx = rx - ex;
        ...
        if (len > 3) {
            const dir = dx > 0 ? 1 : -1;
            const ax2 = rx - dir * 6;
            svg += `<line x1="${ex.toFixed(1)}" y1="${cy2.toFixed(1)}" x2="${ax2.toFixed(1)}" y2="${cy2.toFixed(1)}" stroke="${color}" stroke-width="1.5" stroke-dasharray="3,2" opacity="0.55"/>`;
            svg += `<polygon points="${rx.toFixed(1)},${cy2.toFixed(1)} ${(rx - dir*5).toFixed(1)},${(cy2-3).toFixed(1)} ${(rx - dir*5).toFixed(1)},${(cy2+3).toFixed(1)}" fill="${color}" opacity="0.55"/>`;
        }
    }
    ...
    const cx = toX(stance);
    const cy = axisY - 10;
    svg += `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="5" fill="${color}" opacity="0.9"/>`;
    const labelX = Math.max(16, Math.min(W - 16, cx));
    svg += `<text x="${labelX.toFixed(1)}" y="${(cy - 8).toFixed(1)}" text-anchor="middle" font-size="8" fill="${color}" font-weight="bold">${stance >= 0 ? '+' : ''}${stance.toFixed(2)}</text>`;
...
function showSetup(container: HTMLElement, onStart: (v: SetupValues) => void): void {
        ...
        <div class="ifs-setup">
            ...
            <div class="ifs-setup-grid">
                <div class="ifs-setup-section">
                    <div class="ifs-setup-section-title">Shamer</div>
                ...
                <div class="ifs-setup-section">
                    <div class="ifs-setup-section-title">Drinker</div>
                    <div class="ifs-setup-row">
    ...
    function wireSlider(id: string, valId: string, fmt: (v: number) => string, onChange?: () => void): void {
        ...
        const valEl = container.querySelector<HTMLElement>(`#${valId}`)!;
        const update = () => {
...
function showSim(container: HTMLElement, setup: SetupValues, onReset: () => void): void {
    ...
    const statusEl = container.querySelector('.ifs-status') as HTMLElement;
    const logEl = container.querySelector('.ifs-log') as HTMLElement;
    const arcSvg = container.querySelector('.ifs-arc-svg') as unknown as SVGSVGElement;
    const arcBall = arcSvg.querySelector('.ifs-arc-ball') as unknown as SVGCircleElement;

    // SVG radial gradient for the ball
    ...
    const pauseBtn = container.querySelector<HTMLButtonElement>('#ifs-pause-btn')!;
    const resetBtn = container.querySelector<HTMLButtonElement>('#ifs-reset-btn')!;
    ...
    statusEl.innerHTML = `
        <div class="ifs-status-grid">
            <div class="ifs-part-card" id="ifs-card-a">
                <div class="ifs-part-name">${state.partA.name}</div>
                ...
                <div class="ifs-trust">Self-trust: ${state.partA.selfTrust.toFixed(2)}</div>
            ...
            <div class="ifs-middle">
                ...
                <div class="ifs-trust-val" id="ifs-trust-row-ab">${state.partA.name}→${state.partB.name}: <strong id="ifs-trust-ab"></strong> <span class="ifs-band" id="ifs-band-ab"></span></div>
                <div class="ifs-trust-val" id="ifs-trust-row-ba">${state.partB.name}→${state.partA.name}: <strong id="ifs-trust-ba"></strong> <span class="ifs-band" id="ifs-band-ba"></span></div>
            ...
            <div class="ifs-part-card" id="ifs-card-b">
                <div class="ifs-part-name">${state.partB.name}</div>
                ...
                <div class="ifs-trust">Self-trust: ${state.partB.selfTrust.toFixed(2)}</div>
    ...
    const sbarA = statusEl.querySelector<HTMLElement>('#ifs-sbar-a')!;
    const sbarB = statusEl.querySelector<HTMLElement>('#ifs-sbar-b')!;
    const historyA: number[] = [];
    const historyB: number[] = [];
    let lastHistorySimTime = -Infinity;
    const HISTORY_INTERVAL = 2; // sim-seconds between onion skin snapshots

    function wireBtn(id: string, partId: string, delta: number): void {
        const btn = statusEl.querySelector<HTMLButtonElement>(`#${id}`)!;
        btn.addEventListener('click', () => {
            const rel = partId === state.partA.id ? state.relAB : state.relBA;
            const current = state.conversation.therapistDeltas.get(partId) ?? 0;
            const clamped = clamp(current + delta, -1 - rel.stance, 1 - rel.stance);
            state.conversation.therapistDeltas.set(partId, clamped);
            flashBtnId = id;
...
function shockMagFor(receiverId: string): number {
        const [d1] = nextShockDist(state, receiverId);
        return Math.abs(d1);
    }

    function captureRecordTick(): void {
        const { partA, partB, relAB, relBA, conversation, simTime, lastShock } = state;
        ...
        recording.ticks.push({
            t: +simTime.toFixed(3),
            stanceA: +stanceA.toFixed(4),
            stanceB: +stanceB.toFixed(4),
    ...
    function renderStatus(): void {
        ...
        // Onion skin history
        if (simTime - lastHistorySimTime >= HISTORY_INTERVAL) {
            lastHistorySimTime = simTime;
        ...
        // Yellow hint: which button moves toward regulation (stance → 0)
        // Calm moves stance negative, Activate moves positive.
        // We want to nudge toward 0, so:
        //   stance > +0.3 → Calm (reduce)
        //   stance < -0.3 → Activate (increase)
        ...
        stanceChart(sbarA, stanceA, shockMagFor(partA.id), historyA.slice(0, -1), '#3a6ea5', false, relAB.stance);
        ...
        stanceChart(sbarB, stanceB, shockMagFor(partB.id), historyB.slice(0, -1), '#a53a3a', true, relBA.stance);
    }
    ...
    function renderBall(): void {
        ...
        const wrapEl = container.querySelector('.ifs-status-wrap') as HTMLElement;
        ...
        const neutralHi:   [number,number,number] = [255, 215,  80];
        const neutralMid:  [number,number,number] = [ 60, 190, 190];
        const neutralEdge: [number,number,number] = [130,  60, 200];
        const colorA: [number,number,number] = [ 80, 130, 220];
        const colorB: [number,number,number] = [220,  75,  55];
